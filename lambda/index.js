...
const data = {
  ...
  "lunchSandwich": {
    "ingredients" : [
      {"name": "bread",  "qty": 2, "units": "pieces of"},
      {"name": "ham",    "qty": 1, "units": ""  },
      {"name": "cheese", "qty": 1, "units": "slice of" },
      {"name": "butter", "qty": 1, "units": "tablespoon" }
    ],
    "steps" : [
      "Heat a frying pan on your stove over medium heat.",
      "Add butter in to the frying pan.",
      "Put both pieces of bread on the pan.",
      "Lay the cheese and ham on top of the one piece of bread, and cover with the other piece of bread.",
      "Using a spatula, flip the sandwich to the other side.",
      "Continue flipping until cheese is melted and bread is crunchy",
      "Serve."
    ]
  },
  "dinnerSandwich": {
    "ingredients": [
      {"name": "bread",  "qty": 2, "units": "pieces of"},
      {"name": "steak",    "qty": 1, "units": "lbs"  },
      {"name": "cheese", "qty": 1, "units": "slice of" },
      {"name": "butter", "qty": 1, "units": "tablespoon" },
      {"name": "chimichurri", "qty": 1, "units": "tablespoon" }
    ],
    "steps": [
      "Heat a frying pan on your stove over high heat.",
      "Add 1 tablespoon of olive oil to pan.",
      "Sear steak on both sides. 3 minutes each.",
      "Take the steak out of the pan and rest.",
      "Put bread into the same pan. 1 minute on each side.",
      "Remove bread and add butter to the pan.",
      "Cut steak into strips.",
      "Turn off the heat.",
      "Build the sandwich. Add the sauce from the pan and chimichurri on top",
      "Serve."
    ]
  }
};
...
const handlers = {
  ...
  'IngredientsIntent': function () {
    ...
    var say = "";
    var list = [];
    for (var i = 0; i < data[sandwichType].ingredients.length; i++) {
  ...
  'CookIntent': function () {
    const sandwiches = {
      'breakfast': 'breakfastSandwich',
      'lunch': 'lunchSandwich',
      'dinner': 'dinnerSandwich'
    };

    const slot = this.event.request.intent.slots.sandwich.value;
    if (slot) {
      const sandwichType = sandwiches[slot];
      const savedSandwich = this.attributes['sandwichType'];
      if (sandwichType != savedSandwich) {
        this.attributes['sandwichType'] = sandwichType;
        this.attributes['currentStep'] = 0;
      }
    } else {
      this.attributes['sandwichType'] = 'breakfastSandwich';
    }
  ...
  'AMAZON.NextIntent': function () {
    var sandwichType = this.attributes['sandwichType'];
    var currentStep = incrementStep.call(this, 1);
    var say = 'Step ' + currentStep + ', ' + data[sandwichType].steps[currentStep - 1];
    var reprompt = 'You can say Pause, Stop, or Next.';
    var sayOnScreen = data[sandwichType].steps[currentStep - 1];

    if(currentStep == data[sandwichType].steps.length ) {

      delete this.attributes['currentStep'];
      delete this.attributes['sandwichType'];
